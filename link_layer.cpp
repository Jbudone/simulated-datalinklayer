...
{
// TODO: BUG BUG BUG BUGGY BUGGYNESS
/*if (+(num >> 24 & 0xFF)==48) {
cout << "REALLY!?" << endl;
}
unsigned char c;
c=+(num >> 24 & 0xFF);*/
	buffer[start_position]   = 0;//c;
	buffer[start_position+1] = num >> 16 & 0xFF;
	buffer[start_position+2] = (unsigned int)((num >> 8) & 0xFF);
	buffer[start_position+3] = (unsigned int)((num) & 0xFF);
}

unsigned int Link_layer::read_num_from_buffer(unsigned char buffer[], unsigned int start_position)
{
	unsigned int num=0;
	num = 0;//(unsigned int)buffer[start_position];
...
cout << "FOUND BUG HERE" << endl;
cout << "-[0]: " << (unsigned int)buffer[start_position] << endl;
cout << "-[1]: " << (unsigned int)buffer[start_position+1] << endl;
cout << "-[2]: " << (unsigned int)buffer[start_position+2] << endl;
cout << "-[3]: " << (unsigned int)buffer[start_position+3] << endl;
}
	return num;
}
